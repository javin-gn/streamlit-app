...
DATA_PATH = "./data/Resaleflatprices.csv"
...
col1.metric("Average Price", f"${filtered_df['resale_price'].mean():,.0f}")
col2.metric("Median Price", f"${filtered_df['resale_price'].median():,.0f}")
col3.metric("Median Floor Area", f"{filtered_df['floor_area_sqm'].median():.1f} sqm")
col4.metric("Max Price", f"${filtered_df['resale_price'].max():,.0f}")
...
st.caption(f"🟢 Rerun at: {datetime.now()}")